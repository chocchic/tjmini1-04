...
<body>
	...
	<table>
		<tr>
			<td colspan="18" align="right">
				<button onclick="window.location='/admin/product/writeForm'">상품등록</button>
			</td>
		</tr>
		<tr>
			<td>No.</td>
		...
		<c:forEach var="product" items="${list}">
			<tr>
				...
				<td>
					<button class="deleteBtn" href="${product.pnum}">삭제</button>
				</td>
				<td>
					<button onclick="window.location='/admin/product/writeForm'">상품평조회</button>
				</td>
			</tr>	
		</c:forEach>
	</table>
	...
	<form action="/admin/product/list" method="get" id="pagingForm">
		<input type="hidden" name="pageNum" value="${pageMaker.cri.pageNum}" />
		<input type="hidden" name="listQty" value="${pageMaker.cri.listQty}" />
	...
	<script>
		$(document).ready(function(){
			...
			$("a.paging_btn").click(function(e){
				...
				pagingForm.attr("action", "/admin/product/list"); 
			...
			$(".updateBtn").click(function(){
				...
				pagingForm.attr("action", "/admin/product/modifyForm"); 
			...
			$(".deleteBtn").click(function(){
				...
				pagingForm.append("<input type='hidden' name='pnum' value='"+ $(this).attr("href") + "' />");
				// 폼의 action 속성값 (이동할 주소) content로 변경 
				pagingForm.attr("action", "/admin/product/deleteForm");
				pagingForm.submit();  // 서브밋! 이동!!
			});
			...
			$(".move").click(function(e){
				...
				pagingForm.append("<input type='hidden' name='pnum' value='"+ $(this).attr("href") + "' />");
				// 폼의 action 속성값 (이동할 주소) content로 변경 
				pagingForm.attr("action", "/admin/product/detail"); 
				pagingForm.submit();  // 서브밋! 이동!!
			});
