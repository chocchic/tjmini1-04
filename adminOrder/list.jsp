...
<body>
	...
	<table><!-- 
		<tr>
			<td colspan="18" align="right">
				<button onclick="window.location='/admin/order/writeForm'">주문등록</button>
			</td>
		</tr> -->
		<tr>
			<td>주문 번호</td>
	...
	<form action="/admin/order/list" method="get" id="pagingForm">
		<input type="hidden" name="pageNum" value="${pageMaker.cri.pageNum}" />
		<input type="hidden" name="listQty" value="${pageMaker.cri.listQty}" />
	...
	<script>
		$(document).ready(function(){
			...
			$("a.paging_btn").click(function(e){
				...
				pagingForm.attr("action", "/admin/order/list"); 
			...
			/* $(".updateBtn").click(function(){
				...
				pagingForm.attr("action", "/admin/order/modifyForm"); 
			...
			$(".deleteBtn").click(function(){
				...
				pagingForm.find("input[name='onum']").val($(this).attr("href"));
				// 폼의 action 속성값 (이동할 주소) content로 변경 
				pagingForm.attr("action", "/admin/order/deleteForm");
				pagingForm.submit();  // 서브밋! 이동!!
				$("input[name='onum']").val("");
			}); */
			...
			$(".move").click(function(e){
				...
				pagingForm.find("input[name='onum']").val($(this).attr("href")); 
				// 폼의 action 속성값 (이동할 주소) content로 변경 
				pagingForm.attr("action", "/admin/order/detail"); 
				pagingForm.submit();  // 서브밋! 이동!!
				$("input[name='onum']").val("");
			});
