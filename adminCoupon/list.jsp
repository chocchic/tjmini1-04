...
<body>
	...
	<table>
		<tr>
			<td colspan="18" align="right">
				...
				<button onclick="window.location='/admin/coupon/writeForm'">쿠폰발급</button>
			</td>
		</tr>
		<tr>
	...
	<div align="center">
		<form action="/admin/coupon/list" method="get">
			<select name="type">
				...
				>쿠폰 금액</option>
				<option value="U"
					<c:out value="${pageMaker.cri.type eq 'U' ? 'selected' : ''}"/>
				>쿠폰소유자</option>
			</select>
	...
	<form action="/admin/coupon/list" method="get" id="pagingForm">
		<input type="hidden" name="pageNum" value="${pageMaker.cri.pageNum}" />
		<input type="hidden" name="listQty" value="${pageMaker.cri.listQty}" />
	...
	<script>
		$(document).ready(function(){
			...
			$("a.paging_btn").click(function(e){
				...
				pagingForm.attr("action", "/admin/coupon/list"); 
			...
			$(".updateBtn").click(function(){
				...
				pagingForm.attr("action", "/admin/coupon/modifyForm"); 
			...
			$(".deleteBtn").click(function(){
				...
				pagingForm.find("input[name='cpnum']").val($(this).attr("href"));
				// 폼의 action 속성값 (이동할 주소) content로 변경 
				pagingForm.attr("action", "/admin/coupon/deleteForm");
				pagingForm.submit();  // 서브밋! 이동!!
				$("input[name='cpnum']").val("");
			});
			...
			$(".move").click(function(e){
				...
				// 폼의 action 속성값 (이동할 주소) content로 변경 
				pagingForm.attr("action", "/admin/coupon/detail"); 
				pagingForm.submit();  // 서브밋! 이동!!
				$("input[name='cpnum']").val("");
			});
